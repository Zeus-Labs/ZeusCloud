...
type SignInWithoutMFA struct{}

func (SignInWithoutMFA) UID() string {
	return "cloudwatch/sign_in_without_mfa"
}

func (SignInWithoutMFA) Description() string {
	return "Log metric filter and alarm should exist for management console sign in without MFA"
}
...
func (SignInWithoutMFA) Execute(tx neo4j.Transaction) ([]types.Result, error) {
	records, err := tx.Run(
		...
		t.is_logging AND
		mf.filter_pattern =~ '.*\$\.eventName\s*=\s*"?ConsoleLogin"?.+\$\.additionalEventData\.MFAUsed\s*!=\s*"?Yes"?.*'
...
func (SignInWithoutMFA) ProduceRuleGraph(tx neo4j.Transaction, resourceId string) (neo4j.Result, error) {
	return nil, nil
}
