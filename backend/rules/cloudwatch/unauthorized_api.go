...
func (UnauthorizedAPI) UID() string {
	return "cloudwatch/unauthorized_api"
}

func (UnauthorizedAPI) Description() string {
	return "Log metric filter and alarm should exist for unauthorized API calls"
}
...
func (UnauthorizedAPI) Execute(tx neo4j.Transaction) ([]types.Result, error) {
	records, err := tx.Run(
		...
		t.is_logging AND
		mf.filter_pattern =~ '.*\$\.errorCode\s*=\s*"?\*UnauthorizedOperation"?.+\$\.errorCode\s*=\s*"?AccessDenied\*"?.*'
		...
		END as status,
		CASE WHEN 
			num_valid = 0 THEN 'There is no log metric filter and alarm monitoring an active, multi-region CloudTrail trail for unauthorized API calls.'
			ELSE 'The metric filter ' + valid_mfs[0].filter_name + ' and alarm ' + valid_als[0].arn + ' monitor unauthorized API call events.'
		END as context`,
		nil,
	)
	if err != nil {
...
func (UnauthorizedAPI) ProduceRuleGraph(tx neo4j.Transaction, resourceId string) ([]types.GraphResult, error) {
	return nil, nil
}
