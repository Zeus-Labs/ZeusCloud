...
type RouteTableChanges struct{}

func (RouteTableChanges) UID() string {
	return "cloudwatch/route_table_changes"
}

func (RouteTableChanges) Description() string {
	return "Log metric filter and alarm should exist for route table changes"
}
...
func (RouteTableChanges) Execute(tx neo4j.Transaction) ([]types.Result, error) {
	records, err := tx.Run(
		...
		t.is_logging AND
		mf.filter_pattern =~ '.*\$\.eventName\s*=\s*"?CreateRoute"?.+\$\.eventName\s*=\s*"?CreateRouteTable"?.+\$\.eventName\s*=\s*"?ReplaceRoute"?.+\$\.eventName\s*=\s*"?ReplaceRouteTableAssociation"?.+\$\.eventName\s*=\s*"?DeleteRouteTable"?.+\$\.eventName\s*=\s*"?DeleteRoute"?.+\$\.eventName\s*=\s*"?DisassociateRouteTable"?.*'
		...
		END as status,
		CASE WHEN 
			num_valid = 0 THEN 'There is no log metric filter and alarm monitoring an active, multi-region CloudTrail trail for route table changes.'
			ELSE 'The metric filter ' + valid_mfs[0].filter_name + ' and alarm ' + valid_als[0].arn + ' monitor route table change events.'
		END as context`,
		nil,
	)
	if err != nil {
...
func (RouteTableChanges) ProduceRuleGraph(tx neo4j.Transaction, resourceId string) ([]types.GraphResult, error) {
	return nil, nil
}
