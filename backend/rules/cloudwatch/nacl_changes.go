...
type NACLChanges struct{}

func (NACLChanges) UID() string {
	return "cloudwatch/nacl_changes"
}

func (NACLChanges) Description() string {
	return "Log metric filter and alarm should exist for Network Access Control List (NACL) changes"
}
...
func (NACLChanges) Execute(tx neo4j.Transaction) ([]types.Result, error) {
	records, err := tx.Run(
		...
		t.is_logging AND
		mf.filter_pattern =~ '.*\$\.eventName\s*=\s*"?CreateNetworkAcl"?.+\$\.eventName\s*=\s*"?CreateNetworkAclEntry"?.+\$\.eventName\s*=\s*"?DeleteNetworkAcl"?.+\$\.eventName\s*=\s*"?DeleteNetworkAclEntry"?.+\$\.eventName\s*=\s*"?ReplaceNetworkAclEntry"?.+\$\.eventName\s*=\s*"?ReplaceNetworkAclAssociation"?.*'
		...
		END as status,
		CASE WHEN 
			num_valid = 0 THEN 'There is no log metric filter and alarm monitoring an active, multi-region CloudTrail trail for NACL changes.'
			ELSE 'The metric filter ' + valid_mfs[0].filter_name + ' and alarm ' + valid_als[0].arn + ' monitor NACL change events.'
		END as context`,
		nil,
	)
	if err != nil {
...
func (NACLChanges) ProduceRuleGraph(tx neo4j.Transaction, resourceId string) ([]types.GraphResult, error) {
	return nil, nil
}
