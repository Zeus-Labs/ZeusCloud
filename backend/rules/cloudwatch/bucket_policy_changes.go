...
type BucketPolicyChanges struct{}

func (BucketPolicyChanges) UID() string {
	return "cloudwatch/bucket_policy_changes"
}

func (BucketPolicyChanges) Description() string {
	return "Log metric filter and alarm should exist for S3 bucket policy changes"
}
...
func (BucketPolicyChanges) Execute(tx neo4j.Transaction) ([]types.Result, error) {
	records, err := tx.Run(
		...
		t.is_logging AND
		mf.filter_pattern =~ '.*\$\.eventSource\s*=\s*"?s3.amazonaws.com"?.+\$\.eventName\s*=\s*"?PutBucketAcl"?.+\$\.eventName\s*=\s*"?PutBucketPolicy"?.+\$\.eventName\s*=\s*"?PutBucketCors"?.+\$\.eventName\s*=\s*"?PutBucketLifecycle"?.+\$\.eventName\s*=\s*"?PutBucketReplication"?.+\$\.eventName\s*=\s*"?DeleteBucketPolicy"?.+\$\.eventName\s*=\s*"?DeleteBucketCors"?.+\$\.eventName\s*=\s*"?DeleteBucketLifecycle"?.+\$\.eventName\s*=\s*"?DeleteBucketReplication"?.*'
		...
		END as status,
		CASE WHEN 
			num_valid = 0 THEN 'There is no log metric filter and alarm monitoring an active, multi-region CloudTrail trail for S3 bucket policy changes.'
			ELSE 'The metric filter ' + valid_mfs[0].filter_name + ' and alarm ' + valid_als[0].arn + ' monitor S3 bucket policy change events.'
		END as context`,
		nil,
	)
	if err != nil {
...
func (BucketPolicyChanges) ProduceRuleGraph(tx neo4j.Transaction, resourceId string) ([]types.GraphResult, error) {
	return nil, nil
}
