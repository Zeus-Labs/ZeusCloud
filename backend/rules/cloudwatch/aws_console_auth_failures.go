...
type AWSConsoleAuthFailures struct{}

func (AWSConsoleAuthFailures) UID() string {
	return "cloudwatch/aws_console_auth_failures"
}

func (AWSConsoleAuthFailures) Description() string {
	return "Log metric filter and alarm should exist for AWS Mangement Console authentication failures"
}
...
func (AWSConsoleAuthFailures) Execute(tx neo4j.Transaction) ([]types.Result, error) {
	records, err := tx.Run(
		...
		t.is_logging AND
		mf.filter_pattern =~ '.*\$\.eventName\s*=\s*"?ConsoleLogin"?.+\$\.errorMessage\s*=\s*"?Failed authentication"?.*'
...
func (AWSConsoleAuthFailures) ProduceDisplayGraph(gp types.GraphPathResult) (types.DisplayGraph, error) {
