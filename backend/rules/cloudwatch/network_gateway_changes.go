...
type NetworkGatewayChanges struct{}

func (NetworkGatewayChanges) UID() string {
	return "cloudwatch/network_gateway_changes"
}

func (NetworkGatewayChanges) Description() string {
	return "Log metric filter and alarm should exist for network gateway changes"
}

func (NetworkGatewayChanges) Severity() types.Severity {
	return types.Low
}

func (NetworkGatewayChanges) RiskCategories() types.RiskCategoryList {
...
func (NetworkGatewayChanges) Execute(tx neo4j.Transaction) ([]types.Result, error) {
	records, err := tx.Run(
		...
		t.is_logging AND
		mf.filter_pattern =~ '.*\$\.eventName\s*=\s*"?CreateCustomerGateway"?.+\$\.eventName\s*=\s*"?DeleteCustomerGateway"?.+\$\.eventName\s*=\s*"?AttachInternetGateway"?.+\$\.eventName\s*=\s*"?CreateInternetGateway"?.+\$\.eventName\s*=\s*"?DeleteInternetGateway"?.+\$\.eventName\s*=\s*"?DetachInternetGateway"?.*'
...
func (NetworkGatewayChanges) ProduceRuleGraph(tx neo4j.Transaction, resourceId string) (neo4j.Result, error) {
	return nil, nil
}
