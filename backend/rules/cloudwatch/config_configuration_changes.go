...
type ConfigConfigurationChanges struct{}

func (ConfigConfigurationChanges) UID() string {
	return "cloudwatch/config_configuration_changes"
}

func (ConfigConfigurationChanges) Description() string {
	return "Log metric filter and alarm should exist for AWS Config configuration changes"
}
...
func (ConfigConfigurationChanges) Execute(tx neo4j.Transaction) ([]types.Result, error) {
	records, err := tx.Run(
		...
		t.is_logging AND
		mf.filter_pattern =~ '.*\$\.eventSource\s*=\s*"?config.amazonaws.com"?.+\$\.eventName\s*=\s*"?StopConfigurationRecorder"?.+\$\.eventName\s*=\s*"?DeleteDeliveryChannel"?.+\$\.eventName\s*=\s*"?PutDeliveryChannel"?.+\$\.eventName\s*=\s*"?PutConfigurationRecorder"?.*'
		...
		END as status,
		CASE WHEN
			num_valid = 0 THEN 'There is no log metric filter and alarm monitoring an active, multi-region CloudTrail trail for AWS Config configuration changes.'
			ELSE 'The metric filter ' + valid_mfs[0].filter_name + ' and alarm ' + valid_als[0].arn + ' monitor AWS Config configuration change events.'
		END as context`,
		nil,
	)
	if err != nil {
...
func (ConfigConfigurationChanges) ProduceRuleGraph(tx neo4j.Transaction, resourceId string) (types.GraphPathResult, error) {
	return types.GraphPathResult{}, nil
}

func (ConfigConfigurationChanges) ProduceDisplayGraph(gp types.GraphPathResult) (types.DisplayGraph, error) {
