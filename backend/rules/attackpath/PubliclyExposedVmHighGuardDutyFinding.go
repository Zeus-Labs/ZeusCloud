	...
	records, err := tx.Run(
		...
		CASE
			WHEN has_guardduty_findings THEN (
				'The instance has the following guard duty findings: ' + substring(apoc.text.join(finding_ids, ', '), 0, 1000) + '.'
			)
			ELSE 'The instance does not have any guard duty findings'
		END as context`,
		nil,
	)
	if err != nil {
