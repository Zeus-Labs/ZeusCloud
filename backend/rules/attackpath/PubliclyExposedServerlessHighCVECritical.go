...
type PubliclyExposedServerlessHighCVECritical struct{}

func (PubliclyExposedServerlessHighCVECritical) UID() string {
	return "attackpath/publicly_exposed_serverless_high_permissions_critical_cve"
}

func (PubliclyExposedServerlessHighCVECritical) Description() string {
	return "Publicly exposed serverless function with critical CVE and high permissions."
}

func (PubliclyExposedServerlessHighCVECritical) Severity() types.Severity {
	return types.Critical
}

func (PubliclyExposedServerlessHighCVECritical) RiskCategories() types.RiskCategoryList {
...
func (PubliclyExposedServerlessHighCVECritical) Execute(tx neo4j.Transaction) ([]types.Result, error) {
	records, err := tx.Run(
		...
		WITH a, lambda, public_elbv2_ids,elbv2_cve_ids, collect(role.arn) as high_roles, collect(role.high_reason) as high_reasons
		WITH a, lambda, public_elbv2_ids,elbv2_cve_ids, high_roles, high_reasons,
		...
		a.id as account_id,
		CASE 
			WHEN publicly_exposed_critical_cve_lambda AND is_high THEN 'failed'
			ELSE 'passed'
		END as status,
		CASE 
...
func (PubliclyExposedServerlessHighCVECritical) ProduceRuleGraph(tx neo4j.Transaction, resourceId string) (neo4j.Result, error) {
	...
	records, err := tx.Run(
		...
		OPTIONAL MATCH
			highRolePath=
			(lambda)-[:STS_ASSUME_ROLE_ALLOW]->(role:AWSRole{is_high: True})
		WITH lambda, indirectPaths, elbv2CvePaths, collect(highRolePath) as highRolePaths
		WITH indirectPaths + highRolePaths + elbv2CvePaths AS paths
		RETURN paths`,
		params,
	)
	if err != nil {
