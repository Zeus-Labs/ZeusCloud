...
type PubliclyExposedVmHighIMDSv1Enabled struct{}

func (PubliclyExposedVmHighIMDSv1Enabled) UID() string {
	return "attackpath/publicly_exposed_vm_high_permissions_imdsv1_enabled"
}

func (PubliclyExposedVmHighIMDSv1Enabled) Description() string {
	return "Publicly exposed VM instance with high permissions and IMDSv1 enabled."
}

func (PubliclyExposedVmHighIMDSv1Enabled) Severity() types.Severity {
	return types.Critical
}

func (PubliclyExposedVmHighIMDSv1Enabled) RiskCategories() types.RiskCategoryList {
...
func (PubliclyExposedVmHighIMDSv1Enabled) Execute(tx neo4j.Transaction) ([]types.Result, error) {
	records, err := tx.Run(
		...
		a.id as account_id,
		CASE 
			WHEN publicly_exposed AND e.isimdsv1enabled AND is_high THEN 'failed'
			ELSE 'passed'
		END as status,
		CASE 
...
func (PubliclyExposedVmHighIMDSv1Enabled) ProduceRuleGraph(tx neo4j.Transaction, resourceId string) (neo4j.Result, error) {
