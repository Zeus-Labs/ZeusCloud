...
var CrownJewelMarked = false
var CrownJewelMutex sync.Mutex
...
func ExecuteRulesFunc(postgresDb *gorm.DB, driver neo4j.Driver, rulesToExecute []types.Rule) {

	ExecuteRulesMutex.Lock()
	defer ExecuteRulesMutex.Unlock()

	CrownJewelMutex.Lock()

	if CrownJewelMarked {
		CrownJewelMarked = false
		CrownJewelMutex.Unlock()
	} else {
		CrownJewelMutex.Unlock()
...
func ExecuteRulesLoop(postgresDb *gorm.DB, driver neo4j.Driver, rulesToExecute []types.Rule) {

	for {
		time.Sleep(time.Second * 5)
		ExecuteRulesFunc(postgresDb, driver, rulesToExecute)
	}
...
func CartographyExecutionFunc(postgresDb *gorm.DB, driver neo4j.Driver) {
...
func CartographyExecutionLoop(postgresDb *gorm.DB, driver neo4j.Driver) {
	for {
		time.Sleep(time.Second * 5)
		CartographyExecutionFunc(postgresDb, driver)
	}
}
...
func ResetCartographyStatus(postgresDb *gorm.DB) error {
	statusValues := []string{"RUNNING", "CARTOGRAPHY_PASSED"}
...
func QueueAccountToBeScanned(postgresDb *gorm.DB, accountQueuedToBeScanned string) error {
